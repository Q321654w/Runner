...
namespace Toony_Colors_Pro.Editor.Legacy
{
	namespace Legacy
	{
		public static class TCP2_ShaderGeneratorUtils
		{
			...
			public static string OutputPath
			{
				get
				{
					if (!CustomOutputDir)
					{
						...
						{
							var rootPath = Utils.Utilities.Utils.FindReadmePath(true).Substring("Assets".Length);
			...
			private static string GenerateShaderSource(this TCP2_Config config, TCP2_ShaderGenerator.ShaderGeneratorTemplate template, Shader existingShader = null)
			{
				...
				Utils.Utilities.Utils.AddIfMissing(features, "UNITY_5_4");
#endif
#if UNITY_5_5_OR_NEWER
				Utils.Utilities.Utils.AddIfMissing(features, "UNITY_5_5");
#endif
#if UNITY_5_6_OR_NEWER
				Utils.Utilities.Utils.AddIfMissing(features, "UNITY_5_6");
#endif
#if UNITY_2017_1_OR_NEWER
				Utils.Utilities.Utils.AddIfMissing(features, "UNITY_2017_1");
#endif
#if UNITY_2018_1_OR_NEWER
				Utils.Utilities.Utils.AddIfMissing(features, "UNITY_2018_1");
#endif
#if UNITY_2018_2_OR_NEWER
				Utils.Utilities.Utils.AddIfMissing(features, "UNITY_2018_2");
#endif
				...
				if (!keywords.ContainsKey("SHADER_TARGET"))
				{
					...
					{
						Utils.Utilities.Utils.AddIfMissing(features, "FORCE_SM2");
					}
				}
				...
				else
				{
					...
					if ((depth >= 0 && stack[depth]) || depth < 0)
					{
						...
						switch (parts[0])
						{
							...
							case "enable_kw": Utils.Utilities.Utils.AddIfMissing(features, parts[1]); break;
							case "disable_kw": Utils.Utilities.Utils.RemoveIfExists(features, parts[1]); break;
							case "enable_flag": Utils.Utilities.Utils.AddIfMissing(flags, parts[1]); break;
							case "disable_flag": Utils.Utilities.Utils.RemoveIfExists(flags, parts[1]); break;

							//Keywords that will also be modified in the Config
							case "enable_kw_config":
								Utils.Utilities.Utils.AddIfMissing(features, parts[1]);
								Utils.Utilities.Utils.AddIfMissing(config.Features, parts[1]);
								break;

							case "disable_kw_config":
								Utils.Utilities.Utils.RemoveIfExists(features, parts[1]);
								Utils.Utilities.Utils.RemoveIfExists(config.Features, parts[1]);
								break;
						}
					}
				}
			...
			private static string GetIncludeRelativePath(TCP2_Config config, Shader existingShader)
			{
				...
				if (outputPath == OUTPUT_PATH)
					return INCLUDE_REL_PATH;
				var tcp2includeFile = Utils.Utilities.Utils.GetFileSafe(Application.dataPath, "TCP2_Include.cginc");
