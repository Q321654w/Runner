...
using Toony_Colors_Pro.Editor.Legacy.Legacy;
...
//				mShaderFeatures = new List<string>(shaderImporter.userData.Split(new string[]{","}, System.StringSplitOptions.RemoveEmptyEntries));
		...
		return;
			...
			{
				//Retina display fix
				EditorGUIUtility.labelWidth = Utils.Utilities.Utils.ScreenWidthRetina - 120f;
				...
				if(ShowFilteredProperties(null, props))
				{
					if(!isGeneratedShader)
						Utils.Utilities.Utils.ShaderKeywordToggle("TCP2_DISABLE_WRAPPED_LIGHT", "Disable Wrapped Lighting", "Disable wrapped lighting, reducing intensity received from lights", keywordsList, ref updateKeywords, "Disable Wrapped Lighting");
				...
				if(CategoryFilter("TEXTURE_RAMP"))
				{
					...
					{
						if( Utils.Utilities.Utils.ShaderKeywordToggle("TCP2_RAMPTEXT", "Texture Toon Ramp", "Make the toon ramp based on a texture", keywordsList, ref updateKeywords, "Ramp Style") )
						{
							ShowFilteredProperties("#RAMPT#", props);
						}
						else
				...
				if(CategoryFilter("BUMP"))
				{
					...
					{
						if( Utils.Utilities.Utils.ShaderKeywordToggle("TCP2_BUMP", "BUMP/NORMAL MAPPING", "Enable bump mapping using normal maps", keywordsList, ref updateKeywords, "Normal/Bump map") )
						{
							ShowFilteredProperties("#NORM#", props);
						}
					}
				...
				if(CategoryFilter("SPECULAR", "SPECULAR_ANISOTROPIC"))
				{
					...
					{
						var specular = Utils.Utilities.Utils.HasKeywords(keywordsList, "TCP2_SPEC", "TCP2_SPEC_TOON");
						Utils.Utilities.Utils.ShaderVariantUpdate("Specular", ShaderVariants, ShaderVariantsEnabled, specular, ref updateVariant);

						specular |= Utils.Utilities.Utils.ShaderKeywordRadio("SPECULAR", new[]{"TCP2_SPEC_OFF","TCP2_SPEC","TCP2_SPEC_TOON"}, new[]
							{
						...
						if( specular )
						{
							...
							var specr = Utils.Utilities.Utils.HasKeywords(keywordsList, "TCP2_SPEC_TOON");
							if(specr)
							{
								ShowFilteredProperties("#SPECT#", props);
							}
				...
				if(CategoryFilter("REFLECTION") && !isMobileShader)
				{
					...
					{
						var reflection = Utils.Utilities.Utils.HasKeywords(keywordsList, "TCP2_REFLECTION", "TCP2_REFLECTION_MASKED");
						Utils.Utilities.Utils.ShaderVariantUpdate("Reflection", ShaderVariants, ShaderVariantsEnabled, reflection, ref updateVariant);
					
						reflection |= Utils.Utilities.Utils.ShaderKeywordRadio("REFLECTION", new[]{"TCP2_REFLECTION_OFF","TCP2_REFLECTION","TCP2_REFLECTION_MASKED"}, new[]
							{
				...
				if(CategoryFilter("MATCAP"))
				{
					...
					{
						var matcap = Utils.Utilities.Utils.HasKeywords(keywordsList, "TCP2_MC", "TCP2_MCMASK");
						Utils.Utilities.Utils.ShaderVariantUpdate("Matcap", ShaderVariants, ShaderVariantsEnabled, matcap, ref updateVariant);
					
						matcap |= Utils.Utilities.Utils.ShaderKeywordRadio("MATCAP", new[]{"TCP2_MC_OFF","TCP2_MC","TCP2_MCMASK"}, new[]
							{
				...
				if(CategoryFilter("RIM", "RIM_OUTLINE"))
				{
					...
					{
						var rim = Utils.Utilities.Utils.HasKeywords(keywordsList, "TCP2_RIM");
						var rimOutline = Utils.Utilities.Utils.HasKeywords(keywordsList, "TCP2_RIMO");

						Utils.Utilities.Utils.ShaderVariantUpdate("Rim", ShaderVariants, ShaderVariantsEnabled, rim, ref updateVariant);
						Utils.Utilities.Utils.ShaderVariantUpdate("RimOutline", ShaderVariants, ShaderVariantsEnabled, rimOutline, ref updateVariant);
					
						rim |= rimOutline |= Utils.Utilities.Utils.ShaderKeywordRadio("RIM", new[]{"TCP2_RIM_OFF","TCP2_RIM","TCP2_RIMO"}, new[]
							{
						...
						if( rim || rimOutline )
						{
							...
							{
								if( Utils.Utilities.Utils.ShaderKeywordToggle("TCP2_RIMDIR", "Directional Rim", "Enable directional rim control (rim calculation is approximated if enabled)", keywordsList, ref updateKeywords) )
								{
									ShowFilteredProperties("#RIMDIR#", props);
								}
							}
				...
				if(CategoryFilter("OUTLINE", "OUTLINE_BLENDING"))
				{
					...
					{
						hasOutlineOpaque = Utils.Utilities.Utils.HasKeywords(keywordsList, "OUTLINES");
						hasOutlineBlending = Utils.Utilities.Utils.HasKeywords(keywordsList, "OUTLINE_BLENDING");
						hasOutline = hasOutlineOpaque || hasOutlineBlending;

						Utils.Utilities.Utils.ShaderVariantUpdate("Outline", ShaderVariants, ShaderVariantsEnabled, hasOutlineOpaque, ref updateVariant);
						Utils.Utilities.Utils.ShaderVariantUpdate("OutlineBlending", ShaderVariants, ShaderVariantsEnabled, hasOutlineBlending, ref updateVariant);
					
						hasOutline |= Utils.Utilities.Utils.ShaderKeywordRadio("OUTLINE", new[]{"OUTLINE_OFF","OUTLINES","OUTLINE_BLENDING"}, new[]
							{
					...
					if( hasOutline )
					{
						...
						{
							var outlineTextured = Utils.Utilities.Utils.ShaderKeywordToggle("TCP2_OUTLINE_TEXTURED", "Outline Color from Texture", "If enabled, outline will take an averaged color from the main texture multiplied by Outline Color", keywordsList, ref updateKeywords);
							if(outlineTextured)
							{
								ShowFilteredProperties("#OUTLINETEX#", props);
							}
						}
						Utils.Utilities.Utils.ShaderKeywordToggle("TCP2_OUTLINE_CONST_SIZE", "Constant Size Outline", "If enabled, outline will have a constant size independently from camera distance", keywordsList, ref updateKeywords);
						if( Utils.Utilities.Utils.ShaderKeywordToggle("TCP2_ZSMOOTH_ON", "Correct Z Artefacts", "Enable the outline z-correction to try to hide artefacts from complex models", keywordsList, ref updateKeywords) )
						{
						...
						EditorGUI.indentLevel++;
						Utils.Utilities.Utils.ShaderKeywordRadio(null, new[]{"TCP2_NONE", "TCP2_COLORS_AS_NORMALS", "TCP2_TANGENT_AS_NORMALS", "TCP2_UV2_AS_NORMALS"}, new[]
							{
