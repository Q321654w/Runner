...
namespace Toony_Colors_Pro.Editor
{
	...
	private const string OUTPUT_FOLDER = "/Smoothed Meshes/";
		...
		private Utils.Utilities.Utils.SmoothedNormalsChannel smoothedNormalChannel;
		private Utils.Utilities.Utils.SmoothedNormalsUVType smoothedNormalUVType;
		...
		void OnGUI()
		{
			...
			*/
			...
			smoothedNormalChannel = (Utils.Utilities.Utils.SmoothedNormalsChannel)EditorGUILayout.EnumPopup(TCP2_GUI.TempContent("Vertex Data Target", "Defines where to store the smoothed normals in the mesh; use a target where there isn't any data already."), smoothedNormalChannel);
			EditorGUI.BeginDisabledGroup(smoothedNormalChannel == Utils.Utilities.Utils.SmoothedNormalsChannel.Tangents || smoothedNormalChannel == Utils.Utilities.Utils.SmoothedNormalsChannel.VertexColors);
			smoothedNormalUVType = (Utils.Utilities.Utils.SmoothedNormalsUVType)EditorGUILayout.EnumPopup(TCP2_GUI.TempContent("UV Data Type", "Defines where and how to store the smoothed normals in the target vertex UV channel."), smoothedNormalUVType);
			...
			*/
			...
			using (new EditorGUI.DisabledScope(!mCustomDirectory))
			{
				...
				{
					var outputPath = Utils.Utilities.Utils.OpenFolderPanel_ProjectPath("Choose custom output directory for generated smoothed meshes", mCustomDirectoryPath);
		...
		private Mesh CreateSmoothedMeshAsset(SelectedMesh originalMesh)
		{
			...
			var rootPath = mCustomDirectory ? Application.dataPath + "/" + mCustomDirectoryPath + "/" : Utils.Utilities.Utils.FindReadmePath() + OUTPUT_FOLDER;

			if (!Directory.Exists(rootPath))
				Directory.CreateDirectory(rootPath);

#if UNITY_EDITOR_WIN
			rootPath = rootPath.Replace(mCustomDirectory ? Application.dataPath : Utils.Utilities.Utils.ToSystemSlashPath(Application.dataPath), "").Replace(@"\", "/");
#else
		rootPath = rootPath.Replace(Application.dataPath, "");
			...
			var channel = originalMesh.isSkinned ? Utils.Utilities.Utils.SmoothedNormalsChannel.Tangents : smoothedNormalChannel;
			Mesh newMesh = Utils.Utilities.Utils.CreateSmoothedMesh(originalMesh.mesh, mFormat, channel, smoothedNormalUVType, !originalMesh.isAsset || (originalMesh.isAsset && assetExists));
