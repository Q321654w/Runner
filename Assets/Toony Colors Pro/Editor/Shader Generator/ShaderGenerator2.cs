...
using Toony_Colors_Pro.Editor.Legacy;
	...
	namespace ShaderGenerator
	{
		public class ShaderGenerator2 : EditorWindow
		{
			...
			static TextAsset[] LoadAllTemplates()
			{
				...
				{
					var rootDir = Utils.Utilities.Utils.FindReadmePath();
					systemPath = rootDir.Replace(@"\", "/") + "/Shader Templates 2/";
				}

				if (Directory.Exists(systemPath))
				{
					var txtFiles = Utils.Utilities.Utils.GetFilesSafe(systemPath, "*.txt");

					foreach (var sysPath in txtFiles)
					{
						var unityPath = sysPath;
						if (Utils.Utilities.Utils.SystemToUnityPath(ref unityPath))
						{
...
#pragma warning restore 0649
			...
			void OnGUI_Internal()
			{
				...
				EditorGUILayout.BeginHorizontal();
				{
					if (currentShader == null)
					{
						...
						{
							var newOutputPath = Utils.Utilities.Utils.OpenFolderPanel_ProjectPath("Choose custom output directory for TCP2 generated shaders", ProjectOptions.data.CustomOutputPath);
			...
			static Shader[] GetUserShaders()
			{
				...
				try
				{
					...
					{
						var paths = Utils.Utilities.Utils.GetFilesSafe(rootPath, "*.shader");
						...
						string assetPath = "Assets" + path.Replace(Application.dataPath, "");
							...
							if (shaderImporter != null)
							{
								...
								{
									string osPath = Utils.Utilities.Utils.UnityRelativeToSystemPath(shaderImporter.assetPath);
			...
			static string GetExistingShaderPath(Config config, Shader existingShader)
			{
				...
				if (!unityPath.EndsWith("/"))
					unityPath = unityPath + "/";
				return Utils.Utilities.Utils.UnityRelativeToSystemPath(unityPath);
			}
			...
			internal static bool ContextualHelpBox(string message, string helpTopic = null)
			{
				...
				if (GlobalOptions.data.ShowContextualHelp)
				{
					...
					if (hasLink)
					{
						...
						{
							Application.OpenURL(ShaderGenerator2.DOCUMENTATION_URL + "#" + helpTopic);
						}
					}
				}
